...
# --- НАСТРОЙКИ ---
# Токен берется из переменной окружения, которую ты добавил на Render
...
async def show_profile(callback: CallbackQuery):
    ...
    if user:
        days = (datetime.now().date() - datetime.fromisoformat(user[2]).date()).days
        rank = "Новичок 🌱" if user[3] <= 3 else "Умеющий ☕️" if user[3] <= 10 else "Кофеман 🤎" if user[3] <= 25 else "Легенда кофемании 👑"
        text = f"👤 <b>Профиль:</b> {user[1]}\n🗓 Вы с нами: {max(1, days)} дней\n📦 Заказов: {user[3]}\n🏆 Ранг: {rank}"
...
async def process_address(callback: CallbackQuery, state: FSMContext):
    await state.update_data(address=callback.data.split("_")[1])
    await callback.message.edit_text("✍️ Напишите, что заказать:", reply_markup=get_back_kb())
...
async def process_order_text(message: Message, state: FSMContext):
    ...
    data = await state.get_data()
    await bot.edit_message_text("⏳ Когда забрать заказ?", chat_id=message.chat.id, message_id=data.get("main_msg_id"), reply_markup=get_time_kb())
...
async def process_time(callback: CallbackQuery, state: FSMContext):
    ...
    if time_val == "custom":
        ...
        await finish_order(callback.message, state, time_val)
